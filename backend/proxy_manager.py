...
Format in proxies.txt: one "IP PORT" per line (or "IP:PORT").
"""
            ...
            with self._lock:
                for addr in parsed:
                    if addr not in self._pool:
                        self._pool[addr] = ProxyStats(address=addr)
            if parsed:
                logger.info(f"[Proxy] Loaded {len(parsed)} proxies from {self._file.name}.")
            else:
