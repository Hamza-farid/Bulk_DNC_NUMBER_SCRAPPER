...
with st.sidebar:
    ...
    max_threads = st.slider("Parallel threads", min_value=5, max_value=200, value=40, step=5,
                             help="More threads = faster, but higher chance of rate limiting / bans.")
    ...
    if info:
        est_rate = max(max_threads * 0.7, 1)
        est_time = info["total"] / est_rate
        st.caption(f"Rough estimate: ~{est_rate:.0f}/sec -> ~{est_time:,.0f}s for this range")
    ...
    st.markdown("---")
    st.markdown("### \U0001F511 Session Token")
    st.caption(
        "Open the target site in a real browser, open DevTools -> Network, find a call "
        "to the API, and copy its X-Session-ID header (or session cookie). Paste it below."
    )
    token_input = st.text_area("Paste session token / cookie / JSON", value="", height=70)

    col_a, col_b = st.columns(2)
    with col_a:
        if st.button("Apply token", use_container_width=True):
            if sm.set_token(token_input):
                st.success("Token applied.")
            else:
                st.info("Token cleared (empty input).")
    with col_b:
        if st.button("Auto-refresh (best effort)", use_container_width=True):
            with st.spinner("Attempting automatic session fetch..."):
                ok = sm.refresh()
            if ok:
                st.success("Session refreshed automatically.")
            else:
                st.error(sm.last_error or "Auto-refresh failed - paste a token manually.")

    if st.button("\U0001F50C Test connection", use_container_width=True):
        with st.spinner("Sending a test request..."):
    ...
    if sm.is_valid():
        st.caption(f"Token ({sm.source}): `{sm.token_preview}`")
    else:
        st.caption("No session token set yet.")
    if sm.last_error:
...
if start_clicked:
    if not valid_area:
        ...
        if not numbers:
            ...
            if not sm.is_valid():
                st.error("No session token set. Paste one in the sidebar (or try Auto-refresh), then Test connection before starting.")
                st.stop()
